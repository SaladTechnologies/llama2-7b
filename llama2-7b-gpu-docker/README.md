Run llama2-7b model (text generation open source GPT-3 analog) for inference on server with GPU using zero-dependency Docker image. 
...
```
docker run -p8080:8080 --gpus all --rm -it devforth/llama2-7b-gpu
```
